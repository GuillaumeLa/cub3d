...
int handle_files(char *line , t_config *conf , int identifier)
{
	...
	if (identifier == 2)
		conf->decals.north.path = ft_strdup(filename);
	else if (identifier == 3)
		conf->decals.south.path = ft_strdup(filename);
	else if (identifier == 4)
		conf->decals.west.path = ft_strdup(filename);
	else if (identifier == 5)
		conf->decals.east.path = ft_strdup(filename);
	close(fd);
	return (1);
}
